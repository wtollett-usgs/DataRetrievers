...
def load_config(conf):
    ...
    with open(conf, 'r') as infile:
        for line in infile:
            logger.info('Adding station: %s' % line)
            split = line.split(',')
            print split
    ...
    with open(f, 'r') as infile:
        with open(m, 'w') as outfile:
            for line in infile:
                ...
                ms          = timedelta(microseconds = round(dtobj.microsecond / 1000.0) * 1000.0)
...
def datalogger_to_valve_and_archive(sites):
    ...
    if hour < 0:
        day  = day - 1 if day > 1 else datetime.strptime("%s/12/31" % year - 1, "%Y/%m/%d").timetuple().tm_yday 
        hour = 23
    day  = str(day).zfill(3)
    hour = str(hour).zfill(2)
    logger.info('Grabbing data for year: %s, day: %s, hour: %s' % (year, day, hour))
